...
<template>
  <AtomText v-if="!showEditProfile" class="text-center text-3xl" tag="h1"
    >Personal info</AtomText
  >
  <AtomText
    v-if="!showEditProfile"
    class="text-center mt-3 mb-10 font-extralight"
    tag="p"
    >Basic info, like your name and photo</AtomText
  >
  <div
    v-if="!showEditProfile"
    class="border-0 md:border md:border-gray-400 md:rounded-xl w-full md:w-3/5 md:mx-auto my-5"
  >
    ...
    <div class="px-10 py-6 flex border-t">
      <AtomText class="text-gray-300 text-sm w-2/5 flex items-center"
        >BIO</AtomText
      >
      <AtomText class="w-3/5">{{ user.description }}</AtomText>
    </div>
    <div class="px-10 py-6 flex border-t">
      <AtomText class="text-gray-300 text-sm w-2/5 flex items-center"
        >PHONE</AtomText
